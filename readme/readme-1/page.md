...
Você pode deixar anotações em um documento. Isso pode ser útil para deixar informações para outros usuários que editam posteriormente este documento.
...
Na visualização OCR, o texto é filtrado automaticamente do documento. Isso é usado para reconhecer recursos relevantes, como o código postal, número do contrato, número da fatura e a classificação de um documento.
...
Scripts podem ser criados nas configurações em Tipos de Documento; essas informações serão exibidas aqui.
...
Existem campos que são necessários para edição adicional, esses podem ser editados nas configurações.

Use a dica de ferramenta para descobrir se:&#x20;

* É um campo obrigatório (obrigatório)
* Validação necessária&#x20;
* Baixa confiança
* Divergência total do valor do imposto
...
Aqui você chega à visualização de extração de tabela e tem mais opções para continuar. Por exemplo, no modo de treinamento, aprendendo a tabela.
...
Se uma coluna estiver faltando, você pode criar uma nova coluna aqui. Especifique o título, decida se deve ser um campo obrigatório e o tipo de coluna (isso é importante para o formato correto)
