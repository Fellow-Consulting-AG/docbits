...
## **Créer des points de connexion DocBits**

Ouvrez ION Desk → Connecter → Points de connexion
...
Vous aurez besoin de 4 points de connexion pour ce flux de données, 3 points de connexion API pour les différentes catégories de codes fiscaux (complet, réduit et gratuit) et un point de connexion d'application représentant votre entreprise LN.

&#x20;Pour créer de nouveaux points de connexion, sélectionnez le bouton “+AJOUTER”
...
Sélectionnez “API” en bas de la liste des options
...
C'est ici que vous saisirez tous les détails du point de connexion TaxCode. Pour chacun des trois points de connexion que vous allez créer, faites ce qui suit

* Entrez un Nom : TaxCodeComplet, TaxCodeRéduit, TaxCodeGratuit
* Description : Cela peut être identique au Nom ou similaire
* Importez un compte de service que vous avez créé.

Passez à l'onglet “Documents” et sélectionnez l'icône PLUS pour ajouter le BOD dont nous avons besoin, comme ci-dessous
...
Recherchez le BOD appelé “Sync.LnTaxCode”, cliquez dessus et appuyez sur “OK” pour ajouter le BOD.
...
Passez à la section ION API. Sous Nom de l'appel API, vous pouvez utiliser le nom du BOD, Sync.LnTaxCode
...
Sélectionnez l'API que vous avez configurée pour l'environnement avec lequel vous travaillez et recherchez l'API suivante. Une fois que vous l'avez sélectionné, appuyez sur “OK”.
...
C'est ici qu'il y aura un léger changement pour chaque point de connexion, cela se voit dans les mappages de champs que vous attribuerez à chaque code fiscal car ils diffèrent légèrement.
...
Dans la ligne field_mappings, sous valeur, c'est là que vous mettrez les mappages de champs spécifiques pour le point de connexion de code fiscal spécifique que vous créez (complet, réduit ou gratuit). Ces mappages sont disponibles sur https://docbits.com/doc/field-mappings/.

Le résultat final devrait ressembler à l'image ci-dessus ou être similaire. Une fois cela fait, cliquez sur l'option ENREGISTRER située ici.
...
Accédez à ION Desk → Connecter → Flux de données
...
Cliquez sur “+AJOUTER” et sélectionnez le flux de document
...
Créez le flux de données suivant en faisant glisser et déposer les composants du menu
...
C'est ici que vous sélectionnerez votre entreprise LN, le résultat final devrait ressembler à ce qui suit
...
C'est ici que vous ajouterez le BOD Sync.LnTaxCode précédent, le résultat ressemble à ce qui suit
...
#### **Filtre 1 (haut) – Complet**

Le nom et la description dépendront de l'environnement que vous utilisez et de vos préférences.
...
C'est ici que vous sélectionnez les points de connexion API que vous avez créés précédemment, en sélectionnant l'API dans le menu déroulant "Sélectionner le connecteur API ION".
...
Une fois tout ce qui précède est fait, ENREGISTREZ et ACTIVEZ le flux de données en appuyant sur les boutons suivants
...
## **Déclenchement BOD (LN)**

Ouvrez LN dans Infor
...
Accédez à Commun → Messagerie BOD → Publier les BOD → Publier les données maîtres financières
...
Sélectionnez PLUS et cliquez sur LnTaxCode
...
Cochez la case pour sélectionner les BOD LnTaxCode
...
Revenez à l'onglet OPTIONS, votre configuration devrait ressembler à ce qui suit
...
Lorsque vous souhaitez publier les BODs, sélectionnez PROCESSUS.

Le résultat final devrait vous donner un tableau similaire dans votre environnement DocBits.
