...
* Flujo de datos LN a DocBits funcionando
...
Aquí es donde crearás los dos puntos de conexión necesarios para importar tus datos de LN que se requieren para la Contabilidad Automática.
...
Necesitarás configurar dos puntos de conexión API separados, a saber:

* PlanDeCuentas
* DimensionesFlexFinales

#### **PlanDeCuentas**

La pestaña de conexión para tu punto de conexión de PlanDeCuentas debería verse similar a lo ilustrado a continuación. Dale al punto de conexión un nombre y descripción apropiados, e importa la Cuenta de Servicio que creaste.
...
Necesitarás agregar dos BODs en esta sección para este punto de conexión. Estos son Sync.PlanDeCuentas y Sync.DefinicionCodigo, para agregar estos BODs haz lo siguiente:

**Sync.PlanDeCuentas**

* Haz clic en el ícono PLUS (+)
...
* Cambia a la pestaña ION API, copia el nombre de la API y busca la Llamada a la API presionando el botón SELECCIONAR
...
* En producto, selecciona el Punto Final de API que creaste para el entorno con el que estás trabajando, que creaste en ION API. Busca la siguiente llamada a API, selecciónala y presiona OK.
...
* Aquí es donde configurarás el mapeo de campos para este BOD, tu configuración debería verse como la siguiente. Los mapeos de campos están disponibles en https://docbits.com/doc/field-mappings/.
...
Una vez que hayas completado los pasos anteriores, habrás configurado con éxito el BOD Sync.PlanDeCuentas. Haz clic en el ícono PLUS para agregar el siguiente y último BOD.

**Sync.DefinicionCodigo (DimensionesFlexTotales)**

La pestaña de conexión para tu punto de conexión de DefinicionCodigo debería verse similar a lo ilustrado a continuación. Dale al punto de conexión un nombre y descripción apropiados, e importa la Cuenta de Servicio que creaste.
...
Una vez que hayas completado los pasos anteriores, habrás configurado con éxito el BOD Sync.CodeDefinition para la tabla de datos maestros TotalFlexDimensions.

#### **FinalFlexDimensions**

La pestaña de conexión para tu punto de conexión FinalFlexDimensions debería verse similar a lo ilustrado a continuación. Dale al punto de conexión un nombre y descripción apropiados, así como importa la Cuenta de Servicio que creaste.
...
Necesitarás agregar un BOD en esta sección para este punto de conexión. Este siendo el Sync.CodeDefinition, para agregar este BOD haz lo siguiente:
...
Una vez que hayas completado los pasos anteriores, habrás configurado con éxito el BOD Sync.CodeDefinition para la tabla de datos maestros FinalFlexDimensions.

## **Flujos de Datos**

Necesitarás configurar dos flujos de datos separados para la Contabilidad Automática:
...
Una descripción general de este flujo de datos se muestra a continuación (la cantidad de puntos de conexión de la API de DocBits al final depende de la cantidad de entornos diferentes que estés configurando).
...
La configuración para este punto de conexión depende de la compañía LN que contiene los datos maestros que deseas importar a DocBits, la tuya debería verse similar a lo que se muestra a continuación.
...
Los siguientes documentos deben ser agregados al flujo de datos:
...
Aquí es donde agregarás el punto de conexión de la API de PlanDeCuentas que creaste anteriormente, la configuración para esto debería verse similar a esto
...
El siguiente documento debe ser agregado al flujo de datos:
...
Aquí es donde se agrega el punto de conexión de la API FinalFlexDimensions que creaste anteriormente, la configuración para esto debería verse similar a esto
...
## **Desencadenamiento de BOD (M3)**

Una vez que se haya completado todo lo anterior, deberás navegar a Infor LN y desencadenar los BODs para que los diversos datos maestros que necesitas para la Contabilidad Automática lleguen a DocBits.
...
Desde el menú anterior, en la pestaña del menú izquierdo, selecciona Común → Mensajería BOD → Publicar BODs → Publicar Datos Maestros Financieros. En el siguiente menú encontrarás los BODs de FlexDimensions y ChartOfAccounts para publicar.
...
Selecciona los siguientes BODs para publicar simplemente marcando cada casilla, no es necesario realizar otros cambios ya que queremos publicar todos estos BODs para que los datos maestros estén completos en DocBits.
...
Una vez seleccionados ambos BODs anteriores, navega a la pestaña de Opciones
...
Una vez en el menú de Opciones, selecciona las siguientes opciones y selecciona PROCESAR para publicar los BODs.
...
Una vez hecho esto, deberías ver las tres tablas de datos maestros separadas en tu(s) entorno(s) de DocBits bajo Búsqueda de Datos Maestros:
