...
* Funkcionalan LN ka DocBits protok podataka
...
U Infor-u, otvorite aplikaciju ION Pult. Na levoj kartici, idite na Poveži → Tačke Povezivanja
...
Ovde ćete kreirati dve tačke povezivanja potrebne za uvoz podataka iz LN koji su potrebni za Automatsko knjigovodstvo.

Kliknite na “+DODAJ” da biste kreirali novu tačku povezivanja, izaberite opciju API kao što je prikazano ispod
...
Potrebno je konfigurisati dve odvojene API tačke povezivanja, to su:

* PlanRačuna
* KonačneFleksDimenzije

#### **PlanRačuna**

Kartica povezivanja za vašu tačku povezivanja PlanRačuna treba da izgleda slično kao što je prikazano ispod. Dajte tački povezivanja odgovarajuće ime i opis, kao i uvezite Servisni Nalog koji ste kreirali.
...
Potrebno je dodati dva BOD-a u ovoj sekciji za ovu tačku povezivanja. To su Sync.PlanRačuna i Sync.DefinicijaKoda, da biste dodali ove BOD-ove uradite sledeće:

**Sync.PlanRačuna**

* Kliknite na PLUS (+) ikonu
...
* Prebacite se na karticu ION API, kopirajte ime API-ja i potražite API Poziv pritiskom na dugme IZABERI
...
* Kod proizvoda, izaberite API Krajnju tačku koju ste kreirali za okruženje sa kojim radite, a koji ste kreirali u ION API. Potražite sledeći API poziv, izaberite API poziv i pritisnite OK.
...
* Zatim, prebacite se na karticu Telo Zahteva
...
* Ovde ćete konfigurisati mapiranje polja za ovaj BOD, vaša konfiguracija treba da izgleda kao na sledećoj slici. Mapiranja polja su dostupna na https://docbits.com/doc/field-mappings/.
...
Kada završite sa gore navedenim koracima, uspešno ste konfigurisali BOD Sync.PlanRačuna. Kliknite na PLUS ikonu da biste dodali sledeći i poslednji BOD.

**Sync.DefinicijaKoda (TotalFleksDimenzije)**

Kartica povezivanja za vašu tačku povezivanja DefinicijaKoda treba da izgleda slično kao što je prikazano ispod. Dajte tački povezivanja odgovarajuće ime i opis, kao i uvezite Servisni Nalog koji ste kreirali.
...
* Prebacite se na karticu ION API, kopirajte naziv API-ja i potražite API poziv pritiskom na dugme SELECT
...
* Ovde ćete konfigurisati mapiranje polja za ovaj BOD, vaša konfiguracija treba da izgleda ovako. Mapiranja polja su dostupna na https://docbits.com/doc/field-mappings/.
...
Kada završite sa gore navedenim koracima, uspešno ste konfigurisali Sync.CodeDefinition BOD za tabelu master podataka TotalFlexDimensions.

#### **FinalFlexDimensions**

Kartica veze za vašu tačku veze FinalFlexDimensions treba da izgleda slično kao što je prikazano ispod. Dajte tački veze odgovarajuće ime i opis, kao i uvoz Servisnog naloga koji ste kreirali.
...
Treba da dodate jedan BOD u ovom odeljku za ovu tačku veze. To je Sync.CodeDefinition, da biste dodali ovaj BOD, uradite sledeće:

Kartica veze za vašu tačku veze CodeDefinition treba da izgleda slično kao što je prikazano ispod. Dajte tački veze odgovarajuće ime i opis, kao i uvoz Servisnog naloga koji ste kreirali.

* Izaberite “Pošalji API”
...
Kada završite sa gore navedenim koracima, uspešno ste konfigurisali BOD Sync.CodeDefinition za tabelu master podataka FinalFlexDimensions.

## **Tokovi podataka**

Potrebno je konfigurisati dva odvojena toka podataka za Auto računovodstvo:
...
Pregled ovog toka podataka izgleda kako je prikazano ispod (broj tačaka povezivanja DocBits API zavisi od broja različitih okruženja koje konfigurišete).
...
Konfiguracija ovog priključne tačke zavisi od LN kompanije koja sadrži master podatke koje želite da uvezete u DocBits, vaša konfiguracija treba da izgleda slično kao što je prikazano ispod.
...
Potrebno je dodati sledeće dokumente u tok podataka:
...
#### **DocBits (PlanKontnogOkvira)**

Ovde dodajete tačku povezivanja API PlanKontnogOkvira koju ste ranije kreirali, konfiguracija za ovo treba da izgleda slično kao ova
...
Konfiguracija ove priključne tačke zavisi od LN kompanije koja sadrži master podatke koje želite da uvezete u DocBits, vaša konfiguracija treba da izgleda slično kao što je prikazano ispod.
...
Sledeći dokument treba dodati u tok podataka:
...
Ovde dodajete tačku konekcije API-ja FinalFlexDimensions koju ste ranije kreirali, konfiguracija za ovo treba da izgleda slično ovome
...
Kada završite sve gore navedeno, treba da odete na Infor LN i pokrenete BOD-ove kako bi različiti master podaci koje trebate za Auto računovodstvo stigli u DocBits.
...
Iz gornjeg menija, u levom meniju tabu, izaberite Common → BOD-Messaging → Publish BODs → Publish Financial Master Data. Iz sledećeg menija pronaći ćete BOD-ove FlexDimensions i ChartOfAccounts za objavljivanje.
...
Izaberite sledeće BOD-ove za objavljivanje jednostavno označavanjem svake kutije, nema potrebe za drugim promenama jer želimo da objavimo sve ove BOD-ove kako bi master podaci bili kompletni u DocBits-u.
...
Kada su oba gore navedena BOD-a izabrana, idite na karticu Opcije
...
Kada ste na meniju Opcije, izaberite sledeće opcije i izaberite PROCESS kako biste objavili BOD-ove.
...
Kada završite, trebalo bi da vidite tri odvojene tabele master podataka u vašem DocBits okruženju pod Master Data Lookup:
